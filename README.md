...
- **Platform status**: `GET /admin/status` renders a status dashboard with service tiles and charts.
- **Tenant management portals**:
  - `GET /admin/tenants/{tenantId}` for global admin tenant management.
  - `GET /tenant/{tenantId}` for tenant-scoped administration (users/domains managed per tenant).
...
Tenant admin credentials (per tenant, managed from the Global Admin Center):
...
### Tenant plans
- **Enterprise** tenants have no user cap (treated as unlimited).
- **Starter/Growth** plans enforce a fixed seat limit.
