...
Usage:
  sudo ./scripts/deploy-ubuntu.sh [--clean] [--with-roundcube]

Options:
  --clean   Remove existing service/unit and install directories before reinstalling.
  --with-roundcube  Install Roundcube webmail (requires IMAP gateway in the future).
USAGE
}

CLEAN_INSTALL=false
WITH_ROUNDCUBE=false
...
if [[ "${WITH_ROUNDCUBE}" == "true" ]]; then
  echo "Installing Roundcube webmail..."
  export DEBIAN_FRONTEND=noninteractive
  echo "roundcube-core roundcube/dbconfig-install boolean false" | debconf-set-selections
  apt-get install -y --no-install-recommends apache2 roundcube roundcube-core roundcube-sqlite3
  a2enconf roundcube >/dev/null 2>&1 || true
...
echo "Tenant manage portal: http://127.0.0.1:8080/admin/tenants/default"
echo "Tenant portal: http://127.0.0.1:8080/tenant/default"
echo "Tenant login: http://127.0.0.1:8080/tenant/default/login"
...
check_url() {
  ...
  if [[ "${status}" == "${expected}" ]]; then
    ...
    echo "WARN ${url} returned ${status} (expected ${expected})"
  fi
}

check_url "http://127.0.0.1:8080/healthz" "200"
check_url "http://127.0.0.1:8080/.well-known/jmap" "200"
check_url "http://127.0.0.1:8080/admin/login" "200"
check_url "http://127.0.0.1:8080/admin" "303"
check_url "http://127.0.0.1:8080/admin/status" "303"
check_url "http://127.0.0.1:8080/admin/api/tenants" "303"
check_url "http://127.0.0.1:8080/admin/api/tenants/default/users" "303"
check_url "http://127.0.0.1:8080/admin/tenants/default" "303"
check_url "http://127.0.0.1:8080/tenant/default" "200"
check_url "http://127.0.0.1:8080/tenant/default/login" "200"

if [[ "${WITH_ROUNDCUBE}" == "true" ]]; then
  echo "Roundcube webmail: http://127.0.0.1/roundcube"
fi
