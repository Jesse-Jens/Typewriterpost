...
### How the Current System Works (Initial JMAP API)
The current codebase ships an initial JMAP API service scaffold. This service is intentionally small and is meant to be the first deployable building block in the overall architecture described above.

**Runtime components**
- **JMAP API process:** A Go HTTP server exposing basic endpoints for readiness checks and JMAP discovery.
- **Configuration via environment:** The service reads its listen address and log level from environment variables for easy deployment behind load balancers.

**Endpoints**
- `GET /healthz` returns a plain-text `ok` payload for liveness/readiness checks.
- `GET /.well-known/jmap` returns a minimal JSON discovery document pointing clients to the JMAP endpoint path (`/jmap`).

**Configuration keys**
- `JMAP_API_ADDRESS` (default `:8080`) — bind address/port for the HTTP server.
- `JMAP_LOG_LEVEL` (default `info`) — log verbosity.

This is intentionally minimal and will be expanded with authenticated JMAP endpoints, storage, and push features as the next milestones land.

### Single-Node Ubuntu Deployment (Scripted)
For a quick single-node deployment on Ubuntu (useful for staging or local testing), a full installer script is provided at:

```
scripts/deploy-ubuntu.sh
```

The script builds the JMAP API binary, installs it to `/opt/typewriterpost`, provisions a dedicated system user, writes configuration under `/etc/typewriterpost`, and registers a systemd unit so the service runs on boot.
...
**Clean re-install (removes prior install + systemd unit before reinstalling):**
```
